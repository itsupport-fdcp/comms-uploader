...
server {
    ...
    # 500 MB file plus multipart metadata
...
If a connection still resets during the initial file transfer, check `pm2 logs comms-uploader --lines 100`, the Nginx error log, and `sudo journalctl -k` for an out-of-memory kill. Multipart intake still consumes memory, so the 500 MB limit and sequential processing reduce pressure but do not guarantee that a small instance has sufficient RAM. Verify the instance's free memory and disk space. The application now supports the AWS SDK credential chain, including the EC2 IAM role, when explicit access keys are absent.
