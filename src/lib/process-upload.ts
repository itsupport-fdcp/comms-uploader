...
async function getVideoInfo(filePath: string): Promise<VideoInfo> {
  try {
    ...
    const videoStream = data.streams?.find((s: { codec_type: string; width?: number; height?: number }) => s.codec_type === 'video');
    const width = videoStream?.width;
    const height = videoStream?.height;

    // Check if an audio stream exists
    const hasAudio = data.streams?.some((s: { codec_type: string; width?: number; height?: number }) => s.codec_type === 'audio') ?? false;
...
export async function processUpload(file: UploadInput, onProgress?: (message: string, percent?: number) => void) {
  ...
  try {
    ...
    if (isVideo) {
      ...
      const tempInputPath = file.inputPath;
      ...
      try {
        ...
        if (duration <= 0) {
          ...
          // Upload the staged original when duration cannot be determined.
        } else {
          ...
          for (const [profileIndex, profile] of activeProfiles.entries()) {
            ...
            const encoding = execFilePromise('ffmpeg', [
              ...
              '-maxrate', Math.round(profile.bitrateKbps * 1.5) + 'k',
              '-bufsize', profile.bitrateKbps * 2 + 'k', '-vf', videoFilter,
      ...
      fileContent = await fs.readFile(file.inputPath);
    }
    ...
    if (isHls && hlsTempDir) {
      ...
      const uniqueFolder = `${crypto.randomUUID()}-${sanitizedName.replace(/\.[^/.]+$/, '')}`;
      ...
      const command = new PutObjectCommand({
        Bucket: bucket,
        Key: key,
        Body: fileContent ?? createReadStream(file.inputPath),
      ...
      console.log(`Uploaded single file to S3: ${key} (${compressedSize} bytes)`);
    }
    ...
    if (hlsTempDir) {
      await fs.rm(hlsTempDir, { recursive: true, force: true }).catch(console.error);
    }
  }
}
